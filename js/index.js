//import { authToken } from "./varibles";
...
const updateRepos = (repos, totalCount) => {
  ...
  const totalCountElements = document.querySelectorAll(".Counter");
  totalCountElements.forEach((el) => (el.innerHTML = totalCount));
...
const target = document.getElementById("big_Profile_pics");

let observer = new IntersectionObserver((entries) => {
  const smallProfilePics = document.getElementById("small-profile-pics");

  if (entries[0].boundingClientRect.bottom < 0) {
    smallProfilePics.style.display = "flex";
  } else {
    smallProfilePics.style.display = "none";
  }
});
